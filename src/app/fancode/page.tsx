  ...
  return (
    <main className="min-h-screen bg-[#050505] text-zinc-200 font-sans relative selection:bg-red-500/30">
            ...
            <div className="bg-gradient-to-b from-black/90 via-black/60 to-transparent p-4 flex justify-between items-center z-20 absolute top-0 w-full pointer-events-none">
                ...
                <div className="pointer-events-auto flex items-center gap-3 bg-black/40 backdrop-blur-md p-2 pr-4 rounded-full border border-white/5">
                    ...
                    <div>
                        ...
                        <p className="text-zinc-400 text-[9px] uppercase tracking-wider">{activeMatch.event_category}</p>
                    </div>
                </div>
      ...
      <div className={`transition-all duration-700 ease-in-out ${activeMatch ? 'opacity-0 pointer-events-none blur-2xl scale-95 h-screen overflow-hidden' : 'opacity-100 scale-100'}`}>
        ...
        {/* Matches Grid & Filters (Same as before) */}
        <div className="max-w-7xl mx-auto px-4 mt-8">
            <div className="flex gap-2 overflow-x-auto pb-2 custom-scrollbar">
                {categories.map(cat => <button key={cat} onClick={() => setSelectedCategory(cat)} className={`px-5 py-2 rounded-full text-xs font-bold uppercase tracking-wider border transition-all whitespace-nowrap ${selectedCategory === cat ? "bg-red-600 border-red-500 text-white shadow-lg shadow-red-900/40" : "bg-zinc-900 border-zinc-800 text-zinc-400 hover:bg-zinc-800 hover:text-white"}`}>{cat}</button>)}
            </div>
        </div>
                    ...
                    return (
                        <div key={match.match_id} onClick={() => setActiveMatch(match)} className="group relative bg-[#0f0f11] border border-zinc-800 rounded-2xl overflow-hidden hover:border-red-600/50 transition-all duration-300 cursor-pointer hover:-translate-y-1 hover:shadow-2xl hover:shadow-red-900/10">
                            <div className="aspect-video w-full relative overflow-hidden bg-black">
                                ...
                                <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition duration-300"><div className="w-14 h-14 bg-red-600/90 rounded-full flex items-center justify-center shadow-lg transform scale-50 group-hover:scale-100 transition duration-300 backdrop-blur-sm"><Icons.Play /></div></div>
                            </div>
                            <div className="p-5">
                                <div className="flex justify-between items-start mb-3"><span className="text-[10px] font-bold uppercase tracking-wider text-red-300 bg-red-900/20 px-2 py-1 rounded border border-red-900/30">{match.event_category}</span><div className="flex items-center gap-1 text-[10px] text-zinc-400 font-mono bg-zinc-800/50 px-2 py-1 rounded border border-zinc-700/50"><Icons.Calendar /> {formatDate(match.startTime)}</div></div>
                                ...
                                <div className="mt-4 pt-4 border-t border-zinc-800 flex items-center justify-between"><div className="flex items-center gap-2"><div className={`w-2 h-2 rounded-full ${isLive ? 'bg-red-500 animate-pulse' : 'bg-green-500'}`}></div><span className="text-[10px] text-zinc-400 uppercase tracking-wide font-bold">{isLive ? 'Streaming' : 'Scheduled'}</span></div><button className="text-xs font-bold text-black bg-white hover:bg-red-600 hover:text-white px-4 py-1.5 rounded-lg transition shadow-lg">Watch</button></div>
                            </div>
                        </div>
                    );
                })}
