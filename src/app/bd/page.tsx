...
function LiveTVContent() {
  ...
  // Stats & Anti-Adblock (Simplified for brevity)
  useEffect(() => {
    if (!isClient) return;
    ...
    const incrementTotalVisitors = async () => {
        if (!sessionStorage.getItem('visitedThisSession')) {
            ...
            try { await runTransaction(db, async (transaction) => {
                ...
                else { const newTotal = docSnap.data().total + 1; transaction.update(counterRef, { total: newTotal }); setTotalVisitors(newTotal); }
    ...
    <main className="min-h-screen bg-[#050b14] text-gray-200 font-sans pb-10 select-none selection:bg-cyan-500/30">
      ...
      <div className="max-w-5xl mx-auto px-2 md:px-4 mt-6 space-y-6">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            ...
            <Link href="https://otieu.com/4/7249389" className="col-span-2 md:col-span-1 bg-gradient-to-r from-cyan-900/20 to-blue-900/20 hover:from-cyan-900/30 hover:to-blue-900/30 border border-cyan-500/20 p-3 rounded-xl flex items-center justify-between group transition">
        ...
        <div className="bg-[#111827] p-5 rounded-2xl border border-gray-800 shadow-xl">
          ...
          {loading ? <div className="grid grid-cols-4 md:grid-cols-6 gap-4 animate-pulse">{[...Array(12)].map((_,i) => <div key={i} className="h-24 bg-gray-800 rounded-lg"></div>)}</div> : (
            <div id="channel-grid-container" className="max-h-[60vh] overflow-y-auto pr-2 custom-scrollbar">
