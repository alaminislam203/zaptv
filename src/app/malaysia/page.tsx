...
import { Activity, Search, Heart, AlertTriangle, Users, Tv, Globe } from "lucide-react";
...
const parseM3U = (content: string): any[] => {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('#EXTINF:')) {
      ...
      currentCategory = groupMatch ? groupMatch[1] : "Malaysia";
...
export default function MalaysiaTVPage() {
  ...
  const categories = useMemo(() => {
    const uniqueCats = Array.from(new Set(channels.map(ch => ch.category || "Malaysia")));
    ...
    onSnapshot(doc(db, "settings", "config"), (docSnap) => {
      if (docSnap.exists()) {
        ...
        const fetchM3UPlaylist = async () => {
          try {
            const url = config.malaysiaPlaylistUrl || "https://raw.githubusercontent.com/byte-capsule/skytv/main/malaysia.m3u";
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
        <div className="glass p-12 rounded-[3rem] max-w-md">
          ...
          <p className="text-slate-500 text-sm font-medium">Malaysia section is temporarily offline. Please check back later.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
              ...
              <span className="text-xs font-black text-emerald-500 uppercase tracking-[0.4em]">International</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">
              Malaysia <span className="text-gradient">Live</span>
            </h1>
          </div>
          <div className="flex gap-4">
            ...
            <div className="glass px-8 py-6 rounded-3xl text-center border-emerald-500/20">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Available Channels</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="glass p-8 rounded-[3rem] h-[700px] flex flex-col">
              <div className="mb-8">
                <h3 className="text-xs font-black text-white uppercase italic tracking-widest mb-6">Regional Browser</h3>
                <div className="relative mb-6">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
                  <input
                    type="text"
                    placeholder="Search Malaysia TV..."
                    value={searchQuery}
