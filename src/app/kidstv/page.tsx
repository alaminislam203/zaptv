...
import { Activity, Search, Heart, AlertTriangle, Users, Tv, Baby } from "lucide-react";
...
const parseM3U = (content: string): any[] => {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('#EXTINF:')) {
      ...
      currentCategory = groupMatch ? groupMatch[1] : "Kids";
...
export default function KidsTVPage() {
  ...
  const categories = useMemo(() => {
    const uniqueCats = Array.from(new Set(channels.map(ch => ch.category || "Kids")));
    ...
    onSnapshot(doc(db, "settings", "config"), (docSnap) => {
      if (docSnap.exists()) {
        ...
        const fetchM3UPlaylist = async () => {
          try {
            const url = config.kidsPlaylistUrl || "https://raw.githubusercontent.com/atanuroy22/iptv/refs/heads/main/output/kids.m3u";
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
        <div className="glass p-12 rounded-[3rem] max-w-md">
          ...
          <p className="text-slate-500 text-sm font-medium">Kids section is temporarily offline. Please check back later.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
              ...
              <span className="text-xs font-black text-emerald-500 uppercase tracking-[0.4em]">Fun Zone</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">
              Kids <span className="text-gradient">World</span>
            </h1>
          </div>
          <div className="flex gap-4">
            <div className="glass px-8 py-6 rounded-3xl text-center">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Small Watchers</p>
            </div>
            <div className="glass px-8 py-6 rounded-3xl text-center border-emerald-500/20">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Kids Channels</p>
            </div>
          </div>
        </div>
            ...
            <div className="glass rounded-[3rem] overflow-hidden aspect-video relative border-white/5 shadow-2xl">
              {currentChannel ? (
                ...
                <div className="w-full h-full flex flex-col items-center justify-center gap-6 bg-slate-900/50">
                  ...
                  <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Select a cartoon to begin</p>
                </div>
              )}
            </div>
