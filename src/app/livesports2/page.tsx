...
import { Play, Activity, Search, Shield, Trophy, Users, Tv, Star, ChevronRight, Heart, AlertTriangle } from "lucide-react";
...
const parseM3U = (content: string): any[] => {
  ...
  lines.forEach((line, index) => {
    ...
    if (line.startsWith('#EXTINF:')) {
      ...
      if (currentName) {
        result.push({ id: `m3u-${index}`, name: currentName, logo: currentLogo, category: currentCategory, is_embed: false, sources: [{ label: "Stream 1", url: line }] });
  ...
  const renderPlayer = () => {
    if (!currentChannel) return (
        <div className="w-full h-full flex flex-col items-center justify-center gap-6 bg-slate-900/50">
            ...
            <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Arena is empty</p>
        </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
        <div className="glass p-12 rounded-[3rem] max-w-md">
          ...
          <p className="text-slate-500 text-sm font-medium">Site is temporarily offline. Please check back later.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
              ...
              <span className="text-xs font-black text-emerald-500 uppercase tracking-[0.4em]">Pro Arena</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">
              Sports <span className="text-gradient">Arena</span>
            </h1>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="glass p-8 rounded-[3rem] h-[700px] flex flex-col">
              <div className="mb-8">
                <h3 className="text-xs font-black text-white uppercase italic tracking-widest mb-6">Sports Browser</h3>
