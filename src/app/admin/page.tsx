...
// একাধিক লিংকের জন্য নতুন ইন্টারফেস
...
export default function AdminPanel() {
  ...
  const [siteConfig, setSiteConfig] = useState<SiteSettings>({ 
    ...
    directLinks: [] // ডিফল্ট খালি অ্যারে
  });

  // নতুন লিংক অ্যাড করার জন্য টেম্পোরারি স্টেট
    ...
    onSnapshot(doc(db, "settings", "config"), (docSnap) => {
      if (docSnap.exists()) {
         ...
         setSiteConfig({
             ...
             // যদি ডাটাবেসে directLinks না থাকে, তবে খালি অ্যারে দেবে
  ...
  const addDirectLink = () => {
    ...
    const updatedLinks = [...siteConfig.directLinks, tempLink];
    setSiteConfig({...siteConfig, directLinks: updatedLinks});
    setTempLink({ url: "", label: "" }); // ইনপুট ক্লিয়ার
  };

  // লিংক ডিলেট করা
  const removeDirectLink = (index: number) => {
    const updatedLinks = siteConfig.directLinks.filter((_, i) => i !== index);
    setSiteConfig({...siteConfig, directLinks: updatedLinks});
  };

  const handleSaveSettings = async () => {
    setLoading(true);
    try { await setDoc(doc(db, "settings", "config"), siteConfig); alert("Settings Updated!"); } catch (e) { alert("Error saving settings!"); }
    setLoading(false);
  };
  ...
  const checkAllChannels = async () => {
    if(!confirm("This will check ALL channels. It might take some time.")) return;
  ...
  return (
    <div className="min-h-screen bg-gray-900 text-gray-100 font-sans p-4 md:p-8">
      ...
      {/* --- SETTINGS (UPDATED FOR MULTIPLE LINKS) --- */}
      {activeTab === "settings" && (
        <div className="max-w-2xl mx-auto bg-gray-800 p-6 rounded-lg border border-gray-700 space-y-6">
           ...
           {/* Direct Link Section (List System) */}
           <div className="bg-gray-900 p-4 rounded border border-yellow-600/50">
               ...
               <p className="text-xs text-gray-400 mb-3">Add multiple direct links. One will be shown randomly to the user.</p>
               ...
               <div className="grid gap-2 border-t border-gray-700 pt-3">
                  ...
                  <button onClick={addDirectLink} className="bg-green-600 py-1.5 rounded font-bold text-sm hover:bg-green-500">Add Link to List</button>
               </div>
           </div>
           ...
           <button onClick={handleSaveSettings} disabled={loading} className="w-full bg-purple-600 py-3 rounded font-bold hover:bg-purple-500 shadow-lg">{loading ? "Saving..." : "Save Config"}</button>
        </div>
      )}

      {/* --- CHANNELS, MATCHES, ADS, REPORTS (Same as before) --- */}
      {/* (Code is same, keeping it concise) */}
