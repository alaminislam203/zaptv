    ...
    <div className="min-h-screen bg-[#09090b] text-gray-200 font-sans p-6 md:p-10">
      ...
      {activeTab === 'settings' && (
        <div className="max-w-2xl bg-[#18181b] p-8 rounded-2xl border border-gray-800 animate-fadeIn">
          <h2 className="text-xl font-bold text-indigo-400 mb-6">Global Ad Configuration</h2>
          <div className="space-y-6">
            <div className="grid grid-cols-2 gap-6">
                <div>
                    <label className="text-xs text-gray-500 font-bold uppercase">Min CPC Rate (TK)</label>
                    <input type="number" className="w-full bg-black border border-gray-700 p-3 rounded-lg text-white mt-1" value={config.rates.min_cpc} onChange={e=>setConfig({...config, rates: {...config.rates, min_cpc: Number(e.target.value)}})}/>
                </div>
                <div>
                    <label className="text-xs text-gray-500 font-bold uppercase">Min CPV Rate (TK)</label>
                    <input type="number" className="w-full bg-black border border-gray-700 p-3 rounded-lg text-white mt-1" value={config.rates.min_cpv} onChange={e=>setConfig({...config, rates: {...config.rates, min_cpv: Number(e.target.value)}})}/>
                </div>
            </div>
            
            <div>
                <label className="text-xs text-gray-500 font-bold uppercase">Click Cooldown (Seconds)</label>
                <input type="number" className="w-full bg-black border border-gray-700 p-3 rounded-lg text-white mt-1" value={config.fraud_protection.click_cooldown} onChange={e=>setConfig({...config, fraud_protection: {...config.fraud_protection, click_cooldown: Number(e.target.value)}})}/>
            ...
            <div className="grid grid-cols-2 gap-6">
                <div>
                    ...
                    <input className="w-full bg-black border border-gray-700 p-3 rounded-lg text-white mt-1" value={config.payment_info.bkash} onChange={e=>setConfig({...config, payment_info: {...config.payment_info, bkash: e.target.value}})}/>
                </div>
                <div>
                    <label className="text-xs text-gray-500 font-bold uppercase">Nagad Number</label>
                    <input className="w-full bg-black border border-gray-700 p-3 rounded-lg text-white mt-1" value={config.payment_info.nagad} onChange={e=>setConfig({...config, payment_info: {...config.payment_info, nagad: e.target.value}})}/>
                </div>
            </div>

            <button onClick={saveConfig} disabled={loading} className="w-full bg-indigo-600 py-4 rounded-xl font-bold text-white hover:bg-indigo-500 transition shadow-lg">{loading?"Saving...":"Save Configuration"}</button>
          </div>
        </div>
      )}
      ...
      {activeTab === 'deposits' && (
        <div className="space-y-4 animate-fadeIn">
            {deposits.length === 0 ? <div className="text-gray-500">No pending deposits.</div> : deposits.map(dep => (
                <div key={dep.id} className="bg-[#18181b] p-6 rounded-2xl border border-gray-800 flex justify-between items-center">
                    <div>
                        <h3 className="text-2xl font-bold text-green-400">৳ {dep.amount}</h3>
                        <p className="text-gray-300 font-bold mt-1">{dep.method} <span className="text-gray-600">|</span> TrxID: <span className="text-white font-mono bg-gray-800 px-2 rounded">{dep.trx_id || dep.trnxId}</span></p>
                        <p className="text-xs text-gray-500 mt-2">User ID: {dep.user_id || dep.uid}</p>
                    </div>
                    <div className="flex gap-3">
                        <button onClick={()=>processDeposit(dep, 'approved')} className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2"><Icons.Check/> Approve</button>
                        <button onClick={()=>processDeposit(dep, 'rejected')} className="bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white px-6 py-2 rounded-lg font-bold flex items-center gap-2 transition"><Icons.Cross/> Reject</button>
                    </div>
                </div>
            ))}
        </div>
      ...
      {activeTab === 'campaigns' && (
        <div className="space-y-4 animate-fadeIn">
            {pendingCampaigns.length === 0 ? <div className="text-gray-500">No pending ad requests.</div> : pendingCampaigns.map(cam => (
                <div key={cam.id} className="bg-[#18181b] p-6 rounded-2xl border border-gray-800 flex gap-6">
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-start">
                            ...
                            <div className="text-right">
                                <span className="block text-sm font-bold text-indigo-400">{cam.ad_model || cam.type}</span>
                                <span className="text-xs text-gray-500">Rate: {cam.bid_rate || cam.rate} TK</span>
                            </div>
                        </div>
                        
                        <div className="mt-4 flex flex-wrap gap-4 text-xs text-gray-400 bg-black/30 p-3 rounded-lg border border-gray-800">
                            <span>🌍 Target: {cam.targeting?.countries?.join(", ") || "All"}</span>
                            <span>📱 Device: {cam.targeting?.devices?.join(", ") || "All"}</span>
                            <span>💰 Budget: {cam.total_budget || cam.budget} TK</span>
                        </div>

                        <div className="mt-4 flex gap-3 justify-end">
                            <button onClick={()=>processCampaign(cam.id, 'active')} className="bg-green-600 hover:bg-green-500 text-white px-6 py-2 rounded-lg font-bold text-sm">Approve Ad</button>
                            <button onClick={()=>processCampaign(cam.id, 'rejected')} className="bg-red-600/20 hover:bg-red-600 text-red-400 hover:text-white px-6 py-2 rounded-lg font-bold text-sm transition">Reject</button>
                        </div>
                    </div>
                </div>
            ))}
