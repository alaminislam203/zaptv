"use client";
import React, { useState, useEffect, useRef, Suspense } from "react"; // Suspense ইম্পোর্ট করা হলো
// 1. Firebase imports
import { 
...
// --- মুল লজিক কম্পোনেন্ট (এটি এখন এক্সপোর্ট ডিফল্ট নয়) ---
function LiveTVContent() {
  ...
  if (siteConfig?.maintenanceMode === true) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-gray-900 to-slate-900 flex items-center justify-center px-6 text-white">
        <div className="max-w-md w-full text-center bg-slate-900/70 backdrop-blur border border-gray-800 rounded-2xl p-8 shadow-2xl">
            <div className="mx-auto mb-6 h-14 w-14 flex items-center justify-center rounded-full bg-red-500/10 text-red-500 text-2xl">🛠️</div>
            <h1 className="text-3xl font-bold tracking-wide text-red-500 mb-3">Site Under Maintenance</h1>
            <p className="text-sm sm:text-base text-gray-400 leading-relaxed">We’re upgrading systems to deliver a faster and smoother experience. Please check back shortly.</p>
            <div className="mt-8 flex justify-center"><div className="h-10 w-10 rounded-full border-2 border-gray-700 border-t-red-500 animate-spin"></div></div>
  ...
  return (
    <main className="min-h-screen bg-[#0b1120] text-gray-200 font-sans pb-10 select-none">
      <header className="sticky top-0 z-50 bg-gradient-to-b from-slate-950 to-slate-900 border-b border-gray-800/80 backdrop-blur supports-[backdrop-filter]:bg-slate-950/70">
      ...
      <div className="max-w-4xl mx-auto px-2 md:px-4 mt-4 space-y-4">
        ...
        <div className="max-w-5xl bg-[#0f172a] border border-gray-700 rounded-lg px-4 py-2 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm text-gray-200 font-mono">আমাদের অফিসিয়াল টেলিগ্রাম চ্যানেলে জয়েন করুন আপডেট পেতে</p>
            <Link href="https://t.me/toffeepro"><button className="text-xs sm:text-sm bg-blue-600 hover:bg-blue-500 px-3 py-1.5 rounded-md text-white transition whitespace-nowrap">Join Telegram</button></Link>
        </div>

        <div className="max-w-5xl bg-[#0f172a] border border-gray-700 rounded-lg px-4 py-2 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm text-gray-200 font-mono">৫০০ টির ও বেশি চ্যানেল যোগ করা হয়েছে, দেখুন</p>
            <Link href="/livetv"><button className="text-xs sm:text-sm bg-blue-800 hover:bg-blue-500 px-3 py-1.5 rounded-md text-white transition whitespace-nowrap">Check Now</button></Link>
        </div>
        
        <div className="max-w-5xl bg-[#0f172a] border border-gray-700 rounded-lg px-4 py-2 flex items-center justify-between gap-3">
            <p className="text-xs sm:text-sm text-gray-200 font-mono">আমাদের সাপোর্ট করতে বাটনটিতে ক্লিক করুন</p>
            <Link href="https://otieu.com/4/7249389"><button className="text-xs sm:text-sm bg-sky-500 hover:bg-blue-500 px-3 py-1.5 rounded-md text-white transition whitespace-nowrap">Support Us</button></Link>
        </div>
                ...
                {currentChannel?.sources[activeSourceIndex]?.url.includes(".m3u8") && !currentChannel.is_embed && (
                    <div className="flex items-center gap-2">
                        ...
                        {["plyr", "videojs", "native", "playerjs"].map((type) => (
                            <button key={type} onClick={() => setPlayerType(type as any)} className={`px-3 py-1 rounded-md border transition capitalize ${playerType === type ? "bg-cyan-600 border-cyan-500 text-white" : "bg-gray-800 border-gray-700 text-gray-400 hover:text-gray-200"}`}>{type}</button>
                        ))}
                    </div>
                )}
        ...
        {activeDirectLink && (
            <div className="flex justify-center mt-6">
                <a href={activeDirectLink.url} target="_blank" rel="noopener noreferrer" className="group relative inline-flex items-center gap-2 rounded-full px-8 py-3 font-semibold text-white bg-gradient-to-r from-emerald-600 via-blue-700 to-emerald-700 shadow-lg shadow-emerald-900/40 border border-emerald-400/30 transition-all hover:scale-[1.04] active:scale-[0.98]">
                    <span className="absolute inset-0 rounded-full bg-emerald-400/20 blur-xl opacity-0 group-hover:opacity-100 transition"></span>
