...
import { Play, Activity, Search, Shield, Info, Heart, AlertTriangle, Users, Tv, Globe } from "lucide-react";
    ...
    onSnapshot(doc(db, "settings", "config"), (docSnap) => {
      if (docSnap.exists()) {
        ...
        const fetchM3UPlaylist = async () => {
          try {
            const url = config.arabicTvPlaylistUrl || "https://raw.githubusercontent.com/bugsfreeweb/LiveTVCollector/refs/heads/main/LiveTV/Arabic/LiveTV.m3u";
  ...
  const renderPlayer = () => {
    if (!currentChannel) return (
      <div className="w-full h-full flex flex-col items-center justify-center gap-6 bg-slate-900/50">
        ...
        <p className="text-[10px] font-black text-slate-500 uppercase tracking-[0.3em]">Select an Arabic channel</p>
      </div>
    );
    ...
    return (
      <div className="min-h-screen bg-slate-950 flex items-center justify-center p-6 text-center">
        <div className="glass p-12 rounded-[3rem] max-w-md">
          ...
          <p className="text-slate-500 text-sm font-medium">Arabic TV section is temporarily offline. Please check back later.</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-8 mb-16">
          <div className="text-center md:text-left">
            <div className="flex items-center justify-center md:justify-start gap-3 mb-4">
              ...
              <span className="text-xs font-black text-emerald-500 uppercase tracking-[0.4em]">Global Network</span>
            </div>
            <h1 className="text-4xl md:text-6xl font-black text-white italic uppercase tracking-tighter leading-none">
              Arabic <span className="text-gradient">TV</span>
            </h1>
          </div>
          <div className="flex gap-4">
            ...
            <div className="glass px-8 py-6 rounded-3xl text-center border-emerald-500/20">
              ...
              <p className="text-[10px] font-bold text-slate-500 uppercase tracking-widest">Live Channels</p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
          ...
          <div className="lg:col-span-4 space-y-8">
            <div className="glass p-8 rounded-[3rem] h-[700px] flex flex-col">
              <div className="mb-8">
                <h3 className="text-xs font-black text-white uppercase italic tracking-widest mb-6">Channel Browser</h3>
                <div className="relative mb-6">
                  <Search className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600" />
                  <input
                    type="text"
                    placeholder="Search Arabic channels..."
                    value={searchQuery}
